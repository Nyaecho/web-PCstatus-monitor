...
export default function ArcGauge({ value, label, color }: ArcGaugeProps) {
  const radius = 35;
  const circumference = 2 * Math.PI * radius; // 226.19
  const arcDegree = 240;
  const maxStroke = (arcDegree / 360) * circumference; // 150.8
  const strokeProgress = (Math.max(0, Math.min(100, value)) / 100) * maxStroke;
      ...
      <div className="relative w-24 h-24 flex items-center justify-center">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pt-1">
          <span className="text-base font-bold font-mono tracking-tighter text-slate-800">
            {value.toFixed(1)}
            <span className="text-[10px] font-sans font-normal text-slate-500 ml-0.5">%</span>
          </span>
